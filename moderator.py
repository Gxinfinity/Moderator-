...
BOT_TOKEN = "YAHAN_APNA_TOKEN_DAALEIN" 

# Log Channel ID yahan daalein (Example: )
...
app = Client("A1_ULTRA_FINAL", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
I protect your groups from NSFW, Bad Words, and Spam.
...
def check_nsfw(file_path):
    ...
    # Sticker conversion for AI accuracy
...
async def a1_guardian(client, message: Message):
    ...
    # Media Scan (Photo, Sticker, Video, Zip)
    file_path = None
    try:
        if message.photo or message.sticker or message.video or (message.document and message.document.file_name.endswith('.zip')):
            ...
            if is_bad:
                await message.delete()
                if not is_admin: await a1_instant_destruction(client, message, "NSFW Media Detected")
...
print("🚀 ULTRA A1 PRO MAX IS LIVE...")
app.run()
