...
app = Client("ULTRA_A1_FINAL", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
📝 **Reason:** `{reason}`
🛠️ **Action:** `Mass Cleanup + Permanent Ban`
━━━━━━━━━━━━━━━━━━━━
"""
...
async def a1_mass_ban_cleanup(client, message, reason):
    user_id = message.from_user.id
    try:
        await message.delete() # Triggering message delete
        await client.delete_user_history(message.chat.id, user_id) # Full history clear
        await message.chat.ban_member(user_id) # Permanent ban
...
async def scan_command(client, message):
    ...
    status_msg = await message.reply("🔍 **A1 System: Scanning all members...**")
    bad_users = []
...
async def a1_guardian(client, message: Message):
    ...
    if "t.me/" in text or "http" in text:
        ...
        if LINK_WARNINGS[u_id] >= 3:
            await a1_mass_ban_cleanup(client, message, "Spamming Links (3 Warnings)")
        else:
            await message.reply(f"⚠️ {message.from_user.mention}, **Warning {LINK_WARNINGS[u_id]}/3!** Links are not allowed.")
    ...
    try:
        if message.photo or message.sticker or message.video or message.document:
            if message.sticker and message.sticker.is_animated:
                if not is_admin: await a1_mass_ban_cleanup(client, message, "Animated Sticker")
                return
